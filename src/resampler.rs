...
mod tests {
    ...
    fn test_buffer_boundary_continuity() {
        ...
        assert!(len_diff <= 10, "Length difference too large: {} samples (expected <10)", len_diff);
        ...
        assert!(max_diff <= 10, "Max sample difference too large: {} (expected <=10)", max_diff);
        
        let avg_diff = total_diff as f64 / compare_len as f64;
        assert!(avg_diff < 1.0, "Average difference too large: {:.3} (expected <1.0)", avg_diff);
        ...
        assert!(large_diff_percentage < 1.0, 
    ...
    fn test_position_state_preservation() {
        ...
        assert!(pos_after_first < 1.0, "Fractional position should be less than 1.0");
        ...
        assert!(pos_after_second >= 0.0, "Position should remain non-negative");
        assert!(pos_after_second < 1.0, "Fractional position should remain less than 1.0");
    }
}

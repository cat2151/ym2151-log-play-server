...
use ym2151_log_player_rust::wav_writer::{write_wav, generate_wav, DEFAULT_OUTPUT_FILENAME};
use std::path::Path;
...
fn test_resampler_downsampling() {
    ...
    let output = resampler.resample(&input_samples).expect("Resampling failed");
    ...
    assert!(output_frames >= 700 && output_frames <= 910, 
            "Expected ~750-880 frames, got {}", output_frames);
}
...
fn test_write_wav_simple() {
    ...
    assert!(Path::new(temp_path_str).exists(), "WAV file was not created");
...
fn test_wav_format_verification() {
    ...
    assert_eq!(spec.sample_format, hound::SampleFormat::Int, "Expected integer samples");
...
fn test_resampler_multiple_chunks() {
    ...
    for iteration in 0..10 {
        ...
        assert_eq!(output.len() % 2, 0, "Output not stereo at chunk {}", iteration);
    }
}
...
fn test_expected_output_frames_accuracy() {
    ...
    for (input_frames, expected_output) in test_cases {
        ...
        assert!(
            predicted >= expected_output - tolerance && 
            predicted <= expected_output + tolerance,
            "For {} input frames, expected ~{} output, got {}",
            input_frames, expected_output, predicted
        );
    }
}

...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for ch in 0..8 {
        client::write_register(0, 0x08, ch)?; // Key off all channels
    }

    // Simple melody: play a few notes with timing
    let notes = vec![
        (0, 0x28, 0x48),   // Note C4, channel 0
        (100, 0x30, 0x00), // Octave and note on
        (100, 0x08, 0x78), // Key on channel 0
        (500, 0x08, 0x00), // Key off
        (600, 0x28, 0x4A), // Note D4
        (600, 0x08, 0x78), // Key on
        (1100, 0x08, 0x00), // Key off
        (1200, 0x28, 0x4C), // Note E4
        (1200, 0x08, 0x78), // Key on
        (1700, 0x08, 0x00), // Key off
    ];

    for (time_ms, addr, data) in notes {
        println!(
            "  Writing register 0x{:02X} = 0x{:02X} at +{}ms",
            addr, data, time_ms
        );
        client::write_register(time_ms, addr, data)?;
    }
